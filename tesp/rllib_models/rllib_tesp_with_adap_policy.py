...
from keras_models.keras_tesp_with_adap_policy import KerasTESPWithAdapPolicy


class RLlibTESPWithAdapPolicy(MAMLModel):
    ...
    def prepare(policy_graph, options):
        ...
        options = options["custom_options"]
        mlp_input_dim = policy_graph.observation_space.shape[0] \
            + options["rnn_output_units"]
        mlp_units = (mlp_input_dim, ) \
            + tuple(options["mlp_hidden_units"]) + (output_dim, )
        variables = {}
        with tf.name_scope("variables"):
            variables["policy"] = KerasTESPWithAdapPolicy.get_variables(
                rnn_input_dim,
                options["rnn_units"],
                options["rnn_output_units"],
                options["projection_kernel_init_value"],
                mlp_units,
                options["vf_share_layers"])
        return variables
